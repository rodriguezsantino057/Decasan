...
export async function applyModoPayment(payment: ModoPaymentPayload) {
  ...
  const { data: pedido, error: readError } = await supabaseAdmin
    .from("pedidos")
    .select("id,total,modo_intention_id,confirmation_email_sent_at,transportista,carrier,tracking_number,email,nombre,telefono,direccion,andreani_tracking_number")
    .eq("id", pedidoId)
    .single();
      ...
      try {
        ...
        if (tracking) {
          await supabaseAdmin
            ...
            .eq("id", pedidoId);
          console.info("[zipnova] envio creado via MODO webhook", { pedidoId, tracking });
        }
      } catch (err) {
